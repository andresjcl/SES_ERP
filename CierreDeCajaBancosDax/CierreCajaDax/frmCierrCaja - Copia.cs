    ...
    public partial class frmCierrCaja
    {
        ...
        private void DatosIniciales()
        {
            ...
            if (DatosCierre.FechaIni > fechaLim)
            {
                ...
                txtFechaAl.Value = DateAndTime.Now;
            }
        ...
        {
            c.DaxCombosPtoVtaReg(datosEmpresa.strConxAdcom, ref cboPtoVta, true, datosEmpresa.suc);
            llenarComboPtoVta();
        }

        private void llenarComboPtoVta()
        {
            c.DaxCombosSuc(datosEmpresa.codEmpresa.ToString(), true, datosEmpresa.strConxSyscod, ref cmbSucursal, true);
        }
        ...
        private void txtFechaDel_LostFocus(object sender, EventArgs e)
        {
            if (txtFechaDel.Text != "  /  /")
            {
                if (!Information.IsDate(txtFechaDel.Text))
                {
        ...
        private void txtFechaAl_LostFocus(object sender, EventArgs e)
        {
            if (txtFechaAl.Text != "  /  /")
            {
                if (!Information.IsDate(txtFechaAl.Text))
                {
                    Interaction.MsgBox("Ingrese una fecha válida", MsgBoxStyle.Information);
                    txtFechaAl.Text = "";
                    txtFechaAl.Focus();
                }
                else if (txtFechaDel.Text > txtFechaAl.Text)
                {
        ...
        private void Reporte()
        {
            ...
            CierreDeCaja.ValoresRecibidos.valorEfectivo = valorNumero(txtEfectivo.Text);
            CierreDeCaja.ValoresRecibidos.valorCheques = valorNumero(txtCheques.Text);
            CierreDeCaja.ValoresRecibidos.valorTarjetas = valorNumero(txtTarjetas.Text);
            CierreDeCaja.ValoresRecibidos.nroCheques = valorNumero(txtCantCheques.Text);
            CierreDeCaja.ValoresRecibidos.nroTarjetas = valorNumero(txtCantTarjetas.Text);
            ...
            var FechaD = new ReportParameter("FecDesde", txtFechaDel.Text);
            var FechaH = new ReportParameter("FecHasta", txtFechaAl.Text);
            ...
            var LoteTarjetas = new ReportParameter("LoteTarjetas", NroLoteTarjetas.Text);
            rds.Name = "DataSet1";
        ...
        private void leerOpciones()
        {
            ...
            opcionreporte = "";
            if (cboPtoVta.SelectedValue is null)
        ...
        private void saveImagen(decimal idclave)
        {
            ...
            byte[] bytes = ReportViewer1.LocalReport.Render("PDF", default, mimeType, encoding, extension, streamdIds, Warnings);
        ...
        {
            {
                ...
                txtCantCheques.Text = withBlock.RecibidoNroCheques.ToString();
                txtCantTarjetas.Text = withBlock.RecibidoNroTarjetas.ToString();
                txtCheques.Text = withBlock.RecibidoCheques.ToString();
                txtEfectivo.Text = withBlock.RecibidoEfectivo.ToString();
                txtFechaAl.Value = withBlock.FechaFin;
                txtFechaDel.Value = withBlock.FechaIni;
                txtTarjetas.Text = withBlock.RecibidoTarjetas.ToString();
                cboPtoVta.SelectedValue = withBlock.PuntoDeVenta;
