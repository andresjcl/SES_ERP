...
Public Class frmCierrCaja
    ...
    Dim valorDeUna As Decimal
    Dim valorTransferencia As Decimal
    ...
    Private IdCajaAbierta As Integer = 0
    Private CajaAbierta As String = ""
    Private UsuarioActual As String = datosEmpresa.usr ' tu variable de usuario
    Private CierreConfirmado As Boolean = False
...
#Region "Datos Iniciales"
    ...
    '    ' Verificar si existe una caja abierta para este usuario
    '    Dim query As String = "SELECT TOP 1 IdCaja FROM CierreCaja WHERE Usuario=@usuario AND Estado='Abierto' ORDER BY IdCaja DESC"

    '    Using cn As New SqlConnection(datosEmpresa.strConxAdcom)
    ...
    '                ' Hay una caja abierta, forzar cierre
    '                IdCajaAbierta = Convert.ToInt32(result)
    '                If MessageBox.Show("Se detectó que la caja anterior no se cerró correctamente. ¿Desea cerrarla ahora?",
    '                               "Cierre pendiente", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = DialogResult.Yes Then
    '                    ForzarCierreCaja(IdCajaAbierta)
    '                Else
    '                    MessageBox.Show("No puede continuar sin cerrar la caja anterior.", "Acción requerida", MessageBoxButtons.OK, MessageBoxIcon.Stop)
    '                    Me.Close()
    '                End If
    '            Else
    '                ' No hay caja abierta, registrar apertura
    '                AbrirCaja()
    '            End If
    '        End Using
    '    End Using
    ...
    Private Sub VerificarEstadoCaja()
        Dim query As String = "SELECT TOP 1 IdCaja FROM CierreCaja WHERE Estado='Abierto' AND Equipo=@equipo"
        ...
        Using cn As New SqlConnection(datosEmpresa.strConxAdcom)
            Using cmd As New SqlCommand(query, cn)
                ...
                If result IsNot Nothing Then
                    ...
                    ' Marcar cierre abrupto
                    MarcarCierreAbrupto()

                    ' Cargar valores previos
                    CargarValoresPrevios()

                    'MessageBox.Show("Se detectó un cierre abrupto. Complete el cierre antes de continuar.", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Else
                    ' No hay caja abierta, se crea una nueva
                    AbrirCaja()
                End If
            End Using
        End Using
    ...
    '    Dim query As String = "INSERT INTO CierreCaja (Usuario, FechaHoraApertura, Estado) VALUES (@usuario, @fecha, 'Abierto'); SELECT SCOPE_IDENTITY();"
    ...
    '            cn.Open()
    '            IdCajaAbierta = Convert.ToInt32(cmd.ExecuteScalar())
    '        End Using
    '    End Using
    'End Sub

    'Private Sub CierreDeCaja()
    '    Dim query As String = "UPDATE CierreCaja SET FechaHoraCierre=@fechaCierre, Estado='Cerrado' WHERE IdCaja=@idCaja"

    '    Using cn As New SqlConnection(datosEmpresa.strConxAdcom)
    '        Using cmd As New SqlCommand(query, cn)
    '            cmd.Parameters.AddWithValue("@fechaCierre", DateTime.Now)
    '            cmd.Parameters.AddWithValue("@idCaja", IdCajaAbierta)
    '            cn.Open()
    '            cmd.ExecuteNonQuery()
    '        End Using
    '    End Using
    ...
    Private Sub AbrirCaja()
        If (datosEmpresa.usr.ToUpper() <> "ADMINISTRADOR") Then
            Dim query As String = "INSERT INTO CierreCaja (FechaHoraApertura, Estado, Equipo, Usuario)
                           VALUES (@fecha, 'Abierto', @equipo, @usuario);
            ...
            Using cn As New SqlConnection(datosEmpresa.strConxAdcom)
                Using cmd As New SqlCommand(query, cn)
                    cmd.Parameters.AddWithValue("@fecha", DateTime.Now)
                    cmd.Parameters.AddWithValue("@equipo", Environment.MachineName)
                    cmd.Parameters.AddWithValue("@usuario", datosEmpresa.usr)
                    cn.Open()
                    IdCajaAbierta = Convert.ToInt32(cmd.ExecuteScalar())
                End Using
            End Using
    ...
    Private Sub MarcarCierreAbrupto()
        If (datosEmpresa.usr.ToUpper() <> "ADMINISTRADOR") Then
            Dim query As String = "UPDATE CierreCaja SET CierreAbrupto = 1 WHERE IdCaja=@idCaja AND Estado='Abierto'"
            ...
            ' Opcional: insertar en log
            RegistrarCierreAbrupto()
        End If

    End Sub

    Private Sub RegistrarCierreAbrupto()
        If (datosEmpresa.usr.ToUpper() <> "ADMINISTRADOR") Then
            If (datosEmpresa.usr.ToUpper() <> "ADMINISTRADOR") Then
                Dim query As String = "INSERT INTO LogCierreAbrupto (IdCaja, Equipo, FechaHora)
                           VALUES (@idCaja, @equipo, @fecha)"
    ...
    '    Dim query As String = "SELECT Efectivo, Cheques, Tarjetas, CantCheques, CantTarjetas
    '                       FROM CierreCaja WHERE IdCaja=@idCaja"
    ...
    '                    txtCheques.Text = dr("Cheques").ToString()
    '                    txtTarjetas.Text = dr("Tarjetas").ToString()
    '                    txtCantCheques.Text = dr("CantCheques").ToString()
    '                    txtCantTarjetas.Text = dr("CantTarjetas").ToString()
    '                End If
    '            End Using
    '        End Using
    '    End Using
                ...
                Using cmd As New SqlCommand(query, cn)
                    ...
                    Using dr As SqlDataReader = cmd.ExecuteReader()
                        If dr.Read() Then
                            ...
                            ' ✅ Si hay valores ingresados (mayores a 0), bloqueamos los campos
                            Dim tieneValores As Boolean = (Convert.ToDecimal(If(IsDBNull(dr("Efectivo")), 0, dr("Efectivo"))) > 0) _
                                                    OrElse (Convert.ToDecimal(If(IsDBNull(dr("Cheques")), 0, dr("Cheques"))) > 0) _
                                                    OrElse (Convert.ToDecimal(If(IsDBNull(dr("Tarjetas")), 0, dr("Tarjetas"))) > 0)

                            If tieneValores Then
                                BloquearCampos()
                            End If
                        End If
                    End Using
    ...
    Private Sub ForzarCierreCaja(idCaja As Integer)
        If (datosEmpresa.usr.ToUpper() <> "ADMINISTRADOR") Then
            Dim query As String = "UPDATE CierreCaja SET FechaHoraCierre=@fechaCierre, Estado='Cerrado' WHERE IdCaja=@idCaja"
            ...
            MessageBox.Show("Caja anterior cerrada automáticamente por detección de cierre abrupto.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information)
            AbrirCaja() ' Abrimos la nueva sesión
        End If
    End Sub
    ...
    Private Sub DatosIniciales()
        ...
        proceso = 0
        organizarBotones()
        Text = "CIERRE DE CAJA DE PUNTOS DE VENTA"
        BtnResumenGeneral.Visible = False
        If esgeneral Then
            Text = "CUADRE DE CAJA GENERAL"
...
#Region "Reporte"
    Private Sub Reporte()
        Dim cad As String = ""
        leerOpciones()
        cad = "exec Adc_CieCajDax '" & txtFechaDel.Text + "','" & txtFechaAl.Text & "','" & suc & "','" & ptovta & "','0" & txtEfectivo.Text & "','0" & txtCheques.Text & "','0" & txtTarjetas.Text & txtDeUna.Text & txtTransfer.Text & "'"
        ...
        ValoresRecibidos.valorDeUna = valorNumero(txtDeUna.Text)
        ValoresRecibidos.valorTransferencia = valorNumero(txtTransfer.Text)
        ...
        Dim Usuario As New ReportParameter("Usuario", datosEmpresa.usr.ToUpper)
        Dim Sucursal As New ReportParameter("Sucursal", datosEmpresa.sucNom.ToUpper)
        ...
        Dim ValDeUna As New ReportParameter("ValDeUna", ValoresRecibidos.valorDeUna.ToString())
        Dim ValTransferencia As New ReportParameter("ValTransferencia", ValoresRecibidos.valorTransferencia.ToString())


        rds.Name = "DataSet1"
        rds.Value = SqlDatos.leerTablaAdcom(cad)

        'If datosEmpresa.usr.ToUpper <> "ADMINISTRADOR" Then ReportViewer1.Visible = False Else ReportViewer1.Visible = True
        'If datosEmpresa.usr.ToUpper <> "ADMINISTRADOR" Then SplitContainer1.BackColor = Color.White
        ...
        ReportViewer1.LocalReport.SetParameters(Sucursal)
        ...
        'ReportViewer1.LocalReport.SetParameters(ValDeUna)
        'ReportViewer1.LocalReport.SetParameters(ValTransferencia)
...
#Region "Actualizar"
    ...
    'Private Sub validarPago()
    '    Try
    '        If (datosEmpresa.usr.ToUpper() <> "ADMINISTRADOR") Then
    '            If (txtEfectivo.Text = "") Then MessageBox.Show("El valor de Efectivo no fue ingresado, Caso contrario ingrese 0 ", "Cierre de caja", MessageBoxButtons.OK, MessageBoxIcon.Error) : Return
    '            'If (txtEfectivo.Text = "") Then MessageBox.Show("El valor de Efectivo no fue ingresado ", "Cierre de caja", MessageBoxButtons.OK, MessageBoxIcon.Error) : Return
    '            If (txtCheques.Text = "") Then MessageBox.Show("El valor de Cheques no fue ingresado, Caso contrario ingrese 0  ", "Cierre de caja", MessageBoxButtons.OK, MessageBoxIcon.Error) : Return
    '            If (txtTarjetas.Text = "") Then MessageBox.Show("El valor de Tarjetas no fue ingresado, Caso contrario ingrese 0  ", "Cierre de caja", MessageBoxButtons.OK, MessageBoxIcon.Error) : Return
    '            'ReportViewer1.Visible = False
    '            'SplitContainer1.BackColor = Color.White
    '            txtEfectivo.Enabled = False
    '            txtCheques.Enabled = False
    '            txtCantCheques.Enabled = False
    '            txtTarjetas.Enabled = False
    '            txtCantTarjetas.Enabled = False
    '            Reporte()
    '        Else
    '            Reporte()
    '        End If
    '    Catch ex As Exception
    '        MessageBox.Show("Verifique los valores ingresados", "Error Valores", MessageBoxButtons.OK, MessageBoxIcon.Error)
    '        txtEfectivo.Enabled = True
    '        txtCheques.Enabled = True
    '        txtCantCheques.Enabled = True
    '        txtTarjetas.Enabled = True
    '        txtCantTarjetas.Enabled = True
    '    End Try
    ...
    '        If datosEmpresa.usr.ToUpper() <> "ADMINISTRADOR" Then
    '            ' Validaciones
    '            If String.IsNullOrWhiteSpace(txtEfectivo.Text) Then
    '                MessageBox.Show("El valor de Efectivo no fue ingresado. Caso contrario ingrese 0.", "Cierre de caja", MessageBoxButtons.OK, MessageBoxIcon.Error)
    '                Return False
    '            End If
    '            If String.IsNullOrWhiteSpace(txtCheques.Text) Then
    '                MessageBox.Show("El valor de Cheques no fue ingresado. Caso contrario ingrese 0.", "Cierre de caja", MessageBoxButtons.OK, MessageBoxIcon.Error)
    '                Return False
    '            End If
    '            If String.IsNullOrWhiteSpace(txtTarjetas.Text) Then
    '                MessageBox.Show("El valor de Tarjetas no fue ingresado. Caso contrario ingrese 0.", "Cierre de caja", MessageBoxButtons.OK, MessageBoxIcon.Error)
    '                Return False
    '            End If

    '            ' Validar que sean numéricos
    '            If Not IsNumeric(txtEfectivo.Text) OrElse Not IsNumeric(txtCheques.Text) OrElse Not IsNumeric(txtTarjetas.Text) Then
    '                MessageBox.Show("Los valores deben ser numéricos.", "Cierre de caja", MessageBoxButtons.OK, MessageBoxIcon.Error)
    '                Return False
    '            End If
    ...
    '            ' Bloquear campos para que no sigan editando
    '            txtEfectivo.Enabled = False
    '            txtCheques.Enabled = False
    '            txtCantCheques.Enabled = False
    '            txtTarjetas.Enabled = False
    ...
    Private Function validarPago() As Boolean
        Try
            If datosEmpresa.usr.ToUpper() <> "ADMINISTRADOR" Then
                If String.IsNullOrWhiteSpace(txtEfectivo.Text) OrElse Not IsNumeric(txtEfectivo.Text) Then
                    MessageBox.Show("Ingrese un valor válido para Efectivo.", "Cierre de caja", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Return False
                End If
                If String.IsNullOrWhiteSpace(txtCheques.Text) OrElse Not IsNumeric(txtCheques.Text) Then
                    MessageBox.Show("Ingrese un valor válido para Cheques.", "Cierre de caja", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Return False
                End If
                If String.IsNullOrWhiteSpace(txtTarjetas.Text) OrElse Not IsNumeric(txtTarjetas.Text) Then
                    MessageBox.Show("Ingrese un valor válido para Tarjetas.", "Cierre de caja", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Return False
                End If
                GuardarValoresEnBD()
                BloquearCampos()
    ...
    'Private Sub GuardarValoresEnBD()
    '    Try
    '        Dim query As String = "UPDATE CierreCaja 
    '                            SET Efectivo=@efectivo, Cheques=@cheques, Tarjetas=@tarjetas, CantCheques=@cantCheques, CantTarjetas=@cantTarjetas
    '                            WHERE IdCaja=@idCaja AND Estado='Abierto'"

    '        Using cn As New SqlConnection(datosEmpresa.strConxAdcom)
    '            Using cmd As New SqlCommand(query, cn)
    '                cmd.Parameters.AddWithValue("@efectivo", Convert.ToDecimal(txtEfectivo.Text))
    '                cmd.Parameters.AddWithValue("@cheques", Convert.ToDecimal(txtCheques.Text))
    '                cmd.Parameters.AddWithValue("@tarjetas", Convert.ToDecimal(txtTarjetas.Text))
    '                cmd.Parameters.AddWithValue("@cantCheques", If(String.IsNullOrWhiteSpace(txtCantCheques.Text), 0, Convert.ToInt32(txtCantCheques.Text)))
    '                cmd.Parameters.AddWithValue("@cantTarjetas", If(String.IsNullOrWhiteSpace(txtCantTarjetas.Text), 0, Convert.ToInt32(txtCantTarjetas.Text)))
    '                cmd.Parameters.AddWithValue("@idCaja", IdCajaAbierta)
    '                cn.Open()
    '                cmd.ExecuteNonQuery()
    '            End Using
    '        End Using
    '    Catch ex As Exception
    '        MessageBox.Show("Error al guardar los valores: " & ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
    '    End Try
    'End Sub
...
#End Region
    ...
    Private Sub txtEfectivo_TextChanged(sender As Object, e As EventArgs) Handles txtEfectivo.TextChanged, txtTarjetas.TextChanged, txtCheques.TextChanged
        txtTotal.Text = (Val(txtEfectivo.Text) + Val(txtCheques.Text) + Val(txtTarjetas.Text) + Val(txtDeUna.Text) + Val(txtTransfer.Text)).ToString()
    End Sub
    ...
    'Private Sub CierreDeCaja()
    '    ' 1. Validar si ya está cerrado
    '    If CierreConfirmado = True Then
    ...
    '    ' 2. Registrar el cierre en tu lógica actual
    '    CierreConfirmado = RegistroCierresCaja.RegistrarcierreCaja(DatosCierre)
    ...
    '    ' 3. Actualizar el estado en la tabla CierreCaja
    '    Try
    '        Dim query As String = "UPDATE CierreCaja SET FechaHoraCierre=@fechaCierre, Estado='Cerrado' WHERE IdCaja=@idCaja"
    ...
    '            Using cmd As New SqlCommand(query, cn)
    '                cmd.Parameters.AddWithValue("@fechaCierre", DateTime.Now)
    '                cmd.Parameters.AddWithValue("@idCaja", IdCajaAbierta) ' esta variable debe estar definida al abrir la caja
    '                cn.Open()
    '                cmd.ExecuteNonQuery()
    '            End Using
    '        End Using
    '    Catch ex As Exception
    '        MessageBox.Show("Error al actualizar el estado de la caja en la base de datos: " & ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
    '    End Try
    ...
    Private Sub CierreDeCaja()
        If CierreConfirmado = True Then
            MessageBox.Show("El cierre de caja ya fue realizado.", "Cierre de caja", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Return
        End If

        Dim query As String = "UPDATE CierreCaja 
                           SET FechaHoraCierre=@fecha, Estado='Cerrado' 
                           WHERE IdCaja=@idCaja"
    ...
    Private Sub cargarCierreLeido()
        With DatosCierre
            ...
            txtDeUna.Text = .RecibidoDeUna.ToString()
            txtTransfer.Text = .RecibidoTransferencia.ToString()
    ...
    '            If MessageBox.Show("Confirma salir se registra el cierre de caja con los valores ingresados ? ", "Cierre de caja", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.No Then
    '                e.Cancel = True
    '            Else
    '                CierreDeCaja()
    ...
    '    Else
    '        If (CierreConfirmado = False And esgeneral = False) Then
    '            If MessageBox.Show("Confirma salir no se realizara ningun cierre ? ", "Cierre de caja", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.No Then
    '                e.Cancel = True
    '            End If
    '        End If
    '    End If
    ...
    Private Sub frmCierrCaja_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
        If datosEmpresa.usr.ToUpper <> "ADMINISTRADOR" Then
            If (CierreConfirmado = False And esgeneral = False) Then
                If MessageBox.Show("Confirma salir, se registrará el cierre de caja con los valores ingresados?", "Cierre de caja", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.No Then
                    e.Cancel = True
                Else
                    CierreDeCaja()
                End If
        ...
        Else
            If (CierreConfirmado = False And esgeneral = False) Then
                If MessageBox.Show("Confirma salir? No se realizará ningún cierre.", "Cierre de caja", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.No Then
                    e.Cancel = True
                End If
            End If
        End If
