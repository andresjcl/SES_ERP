...
namespace CtasBan
{
    public partial class CtasBanForm : Form
    {

        //variable que permiten la consulta y visualizacion de datos
        SqlCommand cmd = new SqlCommand();
        SqlDataAdapter misqlDa = new SqlDataAdapter();
        DataSet dato = new DataSet();
        BindingSource valor = new BindingSource();
        ...
        private void CtasBanForm_Load(object sender, EventArgs e)
        {
            ...
            catch
            {
                ...
                MessageBox.Show("No se ha conectado al servidor virtual AdcomDx");
                this.Close();
                this.Dispose();
            }
            conectarAdcom.Close();
        ...
        public void consultarBancos()
        {
            try
            {
                ...
                combo.DaxCombosBancos(coneccionString, ref bancocomboBox);//conectarAdcom.ConnectionString
            }
            catch { }
        }


        public void consultarDatoAdcom(String consulta)
        {
            misqlDa = new SqlDataAdapter(consulta, coneccionString);
            dato = new DataSet();
        ...
        public void mostrarDatos()
        {
            ...
            consultarDatoAdcom(consulta);
            if (dato.Tables[0].Rows.Count > 0)
            {
                abrevtextBox.Text = Convert.ToString(dato.Tables[0].Rows[0]["Bco_Codigo"]).Trim();
                numeroCuentatextBox.Text = Convert.ToString(dato.Tables[0].Rows[0]["Bco_NumCta"]).Trim();
                ctatextBox.Text = Convert.ToString(dato.Tables[0].Rows[0]["Cta_Codigo"]).Trim();
                int index = bancocomboBox.FindString(Convert.ToString(dato.Tables[0].Rows[0]["Bco_Nombre"]).Trim());
                bancocomboBox.SelectedIndex = index;
                String tipo = Convert.ToString(dato.Tables[0].Rows[0]["Bco_TipoCta"]).Trim();
                if(tipo == "C"){
        ...
        //METODO QUE RECIBE LA ACTUALIZACION INSERCION Y ELIMINACION DE REGISTROS
        public void mantenimientoMetodo(String manteniemiento)
        {
            conectarAdcom.Open();
            SqlCommand cmd = new SqlCommand(manteniemiento, conectarAdcom);
            cmd.ExecuteNonQuery();
            conectarAdcom.Close();
        }
        ...
        private void toolStripButton1_Click(object sender, EventArgs e)
        {
            ...
            abreviaturaBanco = Convert.ToString(buscar.Buscar(coneccionString, consulta, "Bco_Codigo", "Bco_Nombre", "", "BusquedaCuentasBancarias"));//
            ...
            if (existeCuentaContable() == 0 ) return;
                ...
                if (existeAbreviatura() != 0)
                {
                    ...
                    + Cta_Codigo + "', Bco_Codigo = '" + Bco_Codigo + "', Bco_Nombre = '" + Bco_Nombre + "', Bco_CodAlex = '" + bancocomboBox.SelectedValue + "'  where Bco_Codigo = '" + abreviaturaBanco.Trim() + "' ";
                        mantenimientoMetodo(actualizar);
                        modoInicial();
                    }
                    catch
                    {
                ...
                else
                {
                    try
                    {
                        ...
                        mantenimientoMetodo(insertar);
                        modoInicial();
                    }
                    catch (Exception ee)
                    {
                        MessageBox.Show("No se pudo guardar orrectamente la nueva cuenta bancaria /n" + ee.Message  );
                    }
                }
        }       
        ...
        private void abrevtextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (char.IsDigit(e.KeyChar) == true)//PERMITE SOLO DIGITOS
            { }
            else if (char.IsLetter(e.KeyChar) == true)//PERMITE SOLO LETRAR
            { }
            else if (e.KeyChar == 8)//PERMITE TECLA DELETE
            { }
            else
