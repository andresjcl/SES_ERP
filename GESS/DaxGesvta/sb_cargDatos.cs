...
namespace DaxGesvta
{
    class sb_cargDatos
    {
        public void CargarDatosDisponibles(string GessConsulta,string strconeccion,ref CheckedListBox camposSeleccion,ref string[] tipoDato ,ref sb_datosMalla datosMalla)
        {                        
        ...
        public void leerDatosaGrid(string Comando, string strconx, String Ordenar, ref sb_datosMalla datosMalla, ref DataGridView lista, ref DataTable Data)
        {
            lista.Columns.Clear();
            ...
            {
                {
                    try
                    {
                        ...
                        if (datosMalla.columna(dColumn.Name).anchoCol != 0) { dColumn.Width = datosMalla.columna(dColumn.Name).anchoCol; };

                        { dColumn.DisplayIndex = datosMalla.columna(dColumn.Name).ordenVertical; };

                        if (datosMalla.columna(dColumn.Name).tipoDato == "DEC")
                        {
                            dColumn.DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                            dColumn.DefaultCellStyle.Format = "###,###,##0.00";
                        }
                        if (datosMalla.columna(dColumn.Name).tipoDato == "DAT")
                        {
        ...
        public DataTable consultarDato(String consulta, string strconeccion)
        {
            ...
            try
            {
                ...
                SqlDataAdapter misqlDa = new SqlDataAdapter("set dateformat dmy;" + consulta ,strconeccion);               
                misqlDa.Fill(Data);
            }
            catch 
            {
