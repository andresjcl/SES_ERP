...
namespace DaxBan
{
    public partial class frmConBan : Form, Interface
    {
        ...
        private DataSet dato;                                       //Conjunto de Datos
        private string consulta;                                    //Consulta para mostrar en el datagridview o reportViewer
        private BindingSource valor;                                //Recurso de datos
        private String banco;                                       //codigo del banco
        private SqlDataAdapter misqlDa;
        private SqlConnection conexion = new SqlConnection();
        private Boolean botonOp = false ;
        //Adaptador de Datos
        private Double datSaldo;                                    //saldo ingresado por el usuario desde el formulario hijo
        ...
        /// <summary>
        /// metodo que recibe la consulta para ejecutar en el sp
        /// </summary>
        ...
        public void consultarDato(String consulta)
        {
            try
            {
                misqlDa = new SqlDataAdapter(consulta, conexion);
                dato = new DataSet();
        ...
        /// <summary>
        /// metodo que calcula el saldo para mostrar en el datagridview en la columna saldo
        /// </summary>
        ...
        /// <summary>
        /// actualiza los datos ingresados en la columna registro del datagridview
        /// </summary>
            ...
            //{
                if (malla.Rows.Count>0)
                {
                    foreach (DataGridViewRow row in malla.Rows)
                    {
                        ...
                        SqlCommand cmd = new SqlCommand(updat, conexion);
                        cmd.ExecuteNonQuery();
                        cambiado  = false;
                        }
                     }
        ...
        public void consultarCuenta()
        {
            ...
            consulta = "select Bco_Codigo from AdcCtaBco where Bco_Nombre = '" + comboBanco.Text + "'";
            consultarDato(consulta);
            if (dato.Tables[0].Rows.Count > 0)
            {
                comboCuentaBanco.DataSource = dato.Tables[0].DefaultView;
                comboCuentaBanco.DisplayMember = "Bco_Codigo";
            }
            else
            {                
        ...
        private void cambiarColor(DataGridViewCell celda)
        {
        ...
        private void comboBanco_KeyPress(object sender, KeyPressEventArgs e)
        {
            e.Handled = true;
        }

        private void comboCuentaBanco_KeyPress(object sender, KeyPressEventArgs e)
        {
            e.Handled = true;
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnFechaIni_Click(object sender, EventArgs e)
        {
            Int32 selectedCellCount = malla.GetCellCount(DataGridViewElementStates.Selected);
            if (selectedCellCount > 0)
            {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnFechaEmi_Click(object sender, EventArgs e)
        {
            Int32 selectedCellCount = malla.GetCellCount(DataGridViewElementStates.Selected);
            if (selectedCellCount > 0)
            {
        ...
        private void checkNoReg_CheckedChanged(object sender, EventArgs e)
        {
            cerrarMalla();            
        }

        private void checkRegBanco_CheckedChanged(object sender, EventArgs e)
        {
            cerrarMalla();
        }

        private void textBoxFechaDesde_TextChanged(object sender, EventArgs e)
        {
            cerrarMalla();
        }

        private void textBoxFechaHasta_TextChanged(object sender, EventArgs e)
        {
            cerrarMalla();
        }

        private void cerrarMalla()
        {
            if (cambiado)
            {
        ...
        private void comboCuentaBanco_SelectedIndexChanged(object sender, EventArgs e)
        {
            cerrarMalla();
         //   consultarCuenta();
        }
        ...
        private void dataGridView1_CellValidated(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //try
            //{
            //    if (dataGridView1.Rows[e.RowIndex].Cells[0].Value.ToString() == "") return;
            //    if (e.RowIndex > 0 & e.ColumnIndex == 0)
            //    {
            //        //if (Convert.ToDateTime(dataGridView1.Rows[e.RowIndex].Cells[0].Value) < Convert.ToDateTime(textBoxFechaDesde.Text) | Convert.ToDateTime(dataGridView1.Rows[e.RowIndex].Cells[0].Value) > Convert.ToDateTime(textBoxFechaHasta.Text)) dataGridView1.Rows[e.RowIndex].Cells[0].Value = "";
            //    }
            //}
            //catch
            //{
        ...
        private Boolean FechaSinValor(Int32 linea )
        {
            bool resp = false;
            try
            {
        ...
        private Boolean celdaValida(DataGridViewCell celda)
        {
        ...
        {
            daxBuscMalla.frmBuscMall  buscador = new daxBuscMalla.frmBuscMall(malla,true,true);
            buscador.ShowDialog();
        }
