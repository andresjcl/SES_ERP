...
namespace DaxComercia
{
    public class Datos
    {
        static internal  string probarConsultaSql(string consulta, string articulo)
        {
            string resp = "";
        ...
        static internal  string CalcularPrecioConsultaSql(string consulta, string articulo)
        {
            string resp = "";
            string ssql = "select top 1 ISNULL( " + consulta + ",0) as valor FROM ";
            ssql += "( select *,19.37 as precioCompraDoc from DaxInvDatCalPrecios where art_codigo = '" + articulo + "' ) as PR";
