...
namespace DaxComercia
{
    public partial class frmAdmPrecio : Form
    {
        ...
        {
            DaxComercia.classVar.classVarIni();
            combPrecio.Items.Clear(); 
            for (int i=0; i<=6; i++)
            {
            combPrecio.Items.Add (DaxComercia.classVar.valorDestino [i]);
            }
            ...
            for (int i=0; i<=5; i++)
            {
                txtValorInicial.Items.Add(DaxComercia.classVar.valorOrigen[i]);
            }
            llenarComboDatos();
        ...
        private void procesarCambios()
        {
            ...
            try
            {
                DaxComercia.classVar.columnaPrecio(combPrecio.Text,txtValorInicial.Text,ref cambiarPrecio,ref precioActual,ref nroPrecio); 
        ...
        {
            frmImpPrecios ff = new frmImpPrecios ();
