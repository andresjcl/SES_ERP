...
namespace DaxDocElectronicos
{
    public class Firma
    {
        ...
        private void GenerarCertificado()
        {
            ...
            X509Certificate certificate = LoadCertificate(strPathCertificado, clvnum.DeCodificar(strPassword).Trim(), out privateKey, out provider);
            s = null;
...
// Guardamos la firma a un fichero en el home del usuario
        ...
        private X509Certificate LoadCertificate(string path, string password, out PrivateKey privateKey, out Provider provider)
        {
            ...
            {
                for (int x = 0; x < certificates.size(); x++)
                {
                    X509Certificate certificate = (X509Certificate)certificates.get(x);
                    bool[] keyUsageInfo = certificate.getKeyUsage();
                    if (keyUsageInfo != null && keyUsageInfo[0])
                    {
                        response = certificate; break;
                    }
                }
                if (response == null)
                {
                    response = (X509Certificate)certificates.get(0);
                }
                // Obtención de la clave privada asociada al certificado  
                privateKey = storeManager.getPrivateKey(response);
                provider = storeManager.getProvider(response);
            }
            return response;
        }
    }
