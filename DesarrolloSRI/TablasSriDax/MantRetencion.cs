    ...
    public partial class MantRetencion : Form
    {
        ...
        { 
            try
            {
                ...
                idDocumentoActual.Sucursal = datosEmpresa.suc;
                //idDocumentoActual.Tipo = cmbDocumento.SelectedValue.ToString(); claseDocDefault
...
//            nombreTablas tabNom = new nombreTablas();
        ...
        private void SubirDatosDocumentoSustento(idDocumento IdDocSustento)
        {
            ...
            MontoBienes = Math.Round(MontoBienesIva + MontoBienesIvaCero, 2);
            MontoServicios = Math.Round(MontoServiciosIva + MontoServiciosIvaCero, 2);

            DataGridViewRow rowrf = malla.CurrentRow;

            if (rowrf.Cells["Doc_OpcDocumento"].Value.ToString() != "" &&
                Convert.ToDouble("0" + rowrf.Cells["Doc_IdClave"].Value.ToString()) > 0)
            {
                if (MessageBox.Show("Desea reemplazar los valores existentes por el nuevo documento ? ",
                    "Registro de retenciones ", MessageBoxButtons.YesNo) == DialogResult.No) return;
            }
            ...
            rowrf.Cells["Doc_Numero"].Value = IdDocSustento.numero;
            rowrf.Cells["Doc_IdClave"].Value = Convert.ToDecimal(IdDocSustento.idClave);
            rowrf.Cells["Doc_CodSri"].Value = "";
            rowrf.Cells["CodigoRetencion"].Value = "";

            rowrf.Cells["TipoRetencion"].Value = "RetFuente";

            rowrf.Cells["BaseRetencion"].Value = (MontoBienesIva + MontoServiciosIva);
            rowrf.Cells["BaseConIva"].Value = (MontoBienesIva + MontoServiciosIva);

            rowrf.Cells["BaseExcentaIva"].Value = (MontoBienesIvaCero + MontoServiciosIvaCero);
            rowrf.Cells["BaseIvaCero"].Value = 0;
        ...
        private void totalizar()
        {
            ...
            if (malla.CurrentCell != null)
            {
                ...
                if (row.Cells["BaseRetencion"].Value != null && row.Cells["PorcRetencion"].Value != null && !string.IsNullOrWhiteSpace(row.Cells["PorcRetencion"].Value.ToString()))
                {
                    ...
                    ValReten = baseRet * porcRet / 100;
                }
                else
                {
                    ValReten = 0; // o puedes dejarlo sin asignar, o mostrar mensaje
                }
            ...
            try
            {
                if (idDocumentoActual.idClave == 0)
                {
                    ...
                    if (res.Substring(0, 3) != "ERR")
                    {
                        ...
                        AuditSis.registrar.registraEventoDoc(datosEmpresa.strConIniSis, datosEmpresa.codEmpresa.ToString(), datosEmpresa.usr, "MDD", Environment.MachineName, AuditSis.registrar.EvntCrear, idDocumentoActual.Sucursal, idDocumentoActual.Tipo, idDocumentoActual.numero.ToString(), DatosDocumento.Doc_valor.ToString());
                    }
                ...
                else
                {
                    ...
                    {
                        DatosRetencion.GuardarDetalleDoc(DatosDocumento, malla);
                        AuditSis.registrar.registraEventoDoc(datosEmpresa.strConIniSis, datosEmpresa.codEmpresa.ToString(), datosEmpresa.usr, "MDD", Environment.MachineName, AuditSis.registrar.EvntModifica, idDocumentoActual.Sucursal, idDocumentoActual.Tipo, idDocumentoActual.numero.ToString(), DatosDocumento.Doc_valor.ToString());
                    }
