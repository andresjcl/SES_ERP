...
Public Class Form1
    ...
    Private Function Exportar_Excel(ByVal MallaDatos As DataGridView, ByVal Empresa As String, ByVal titulo As String) As Boolean
        ...
        With MallaDatos
            ...
            For Each Columna As DataGridViewColumn In MallaDatos.Columns
                idCol = Columna.Index + 1
                tipoColumna = Columna.ValueType.ToString()
                formato(HojaTrabajoExcel, 3, idCol, 3, idCol, 1)
                HojaTrabajoExcel.Cells(3, idCol).Value = Columna.HeaderText
                For Fila = 0 To .RowCount - 1
                    idLin = Fila + 4
                    Try
                        If Not IsNothing(.Rows(Fila).Cells(Columna.Index).Value) Then
                            If tipoColumna = "System.String" Then
                                If IsNumeric(.Rows(Fila).Cells(Columna.Index).Value) Then
                                    HojaTrabajoExcel.Cells(idLin, idCol).Value = "'" + .Rows(Fila).Cells(Columna.Index).Value.ToString()
                                ElseIf tipoColumna = "System.Date" Or tipoColumna = "System.DateTime" Then
                                    HojaTrabajoExcel.Cells(idLin, idCol).Value = .Rows(Fila).Cells(Columna.Index).FormattedValue()
                                Else
                                    HojaTrabajoExcel.Cells(idLin, idCol).Value = .Rows(Fila).Cells(Columna.Index).Value.ToString()
                                End If
                            Else
                                HojaTrabajoExcel.Cells(idLin, idCol).Value = .Rows(Fila).Cells(Columna.Index).Value
                            End If
                        End If
                    Catch
                    End Try
    ...
    Private Sub Exportar_Word(ByVal datos As DataGridView, ByVal Empresa As String, ByVal titulo As String)
        ...
        Parrafo = Documento.Tables.Add(Documento.Range(0, 0), filas + 2, datos.Columns.Count, Enabled)
        ...
        Catch            'o_Libro.Close()
            MsgBox("Exportación a Word correcta " & vbCr & "Su archivo NO PUDO grabarse todavía ", MsgBoxStyle.Information)
        End Try       ' -- Terminar instancias   
        o_Word.Visible = True
        o_Word = Nothing
        Documento = Nothing
    ...
    Private Sub ExportarDatosPDF(ByVal document As Document, ByVal datos As DataGridView, ByVal emp As String, tit1 As String, tit2 As String)
        ...
        Dim encabezado As New Paragraph(emp, New Font(Font.Name = "Tahoma", 14, Font.Bold))
        Dim texto As New Phrase(tit1, New Font(Font.Name = "Tahoma", 12, Font.Bold))
        Dim texto2 As New Phrase(tit2, New Font(Font.Name = "Tahoma", 12, Font.Bold))
