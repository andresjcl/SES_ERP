...
#Region "Datos Iniciales"
    ...
    Private Sub btnNoAgrupados_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNoAgrupados.Click
        ...
        If btnNoAgrupados.Text = "No Agrupados" Then
            llenarMalla(nivel, ActivofijoInventario, cod, True)
        ...
        If d = "I" Then
            Select Case niv
                ...
                Case Else
                    campo = "Art_categor"
            End Select
            ssql = "SELECT AdcArt.Art_codigo as Codigo, AdcArt.Art_nombre as Nombre"
            ssql += "," & campo & " as NivCod, AdcNiv.Niv_nombre "
            ssql += " FROM AdcArt LEFT OUTER JOIN  AdcNiv"
            ssql += " ON " & campo & " = AdcNiv.Niv_abrevia"
            If sinGrup = True Then ssql += " where " & campo & " =''"
            ssql += " order by art_codigo "
        Else
            'campo = IIf(niv = 1, "Categoria", IIf(niv = 2, "Clase", IIf(niv = 3, "Grupo", "Subgrupo")))
            'ssql = "select Codigo, Nombre," & campo & " as NivCod,ct.Niv_nombre  from adcacf left join"
            'ssql += " (select Niv_abrevia ,Niv_nombre,Niv_categor  from AdcNivAcf"
            'ssql += " )ct on(ct.Niv_categor=" & niv & " and ct.Niv_abrevia='" & codigo & "' )"
            Select Case niv
                Case 2
                    campo = "clase"
                Case 3
                ...
                Case Else
                    campo = "categoria"
            End Select
            ssql = "SELECT Codigo, Nombre"
            ssql += "," & campo & " as NivCod, AdcNiv.Niv_nombre "
            ssql += " FROM Adcacf LEFT OUTER JOIN  AdcNiv"
            ssql += " ON " & campo & " = AdcNiv.Niv_abrevia"
            If sinGrup = True Then ssql += " where " & campo & " ='' "
...
#Region "Agrupar"
    Private Sub btnAgrupar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAgrupar.Click
        Dim cmp As String = ""
        cmp = IIf(ActivofijoInventario = "I", IIf(nivel = 1, "Art_categor", IIf(nivel = 2, "Art_clase", IIf(nivel = 3, "Art_grupo", "Art_subgrup"))), IIf(nivel = 1, "Categoria", IIf(nivel = 2, "Clase", IIf(nivel = 3, "Grupo", "Subgrupo")))).ToString()
        If Malla.SelectedRows.Count > 0 Then
            If MsgBox("Confirma cambiar de categoria a los " & etiqueta & " seleccionados, estos serán registrados en " & Nomb, MsgBoxStyle.YesNo) = vbYes Then
                Agrupar(IIf(ActivofijoInventario = "I", "AdcArt", "AdcAcf").ToString(), cmp, cod, IIf(ActivofijoInventario = "I", "Art_Codigo", "Codigo").ToString())
                limpiar()
                llenarMalla(nivel, ActivofijoInventario, cod, False)
                actualizaNandinaHts(nivel)
            End If
        Else
...
#End Region
    ...
    Public Sub AgruparArt(ByVal nivelC As Int32, ByVal ActivoInventario As String, ByVal codCate As String, ByVal nombre As String)
        nivel = nivelC
        ActivofijoInventario = ActivoInventario
        If ActivoInventario = "A" Then
            Me.Text = "ADCOMDAX - AGRUPAR ACTIVOS FIJOS" : tabla = "AdcnivAcf"
            etiqueta = "Activos"
        Else
            Me.Text = "ADCOMDAX - AGRUPAR ARTÍCULOS DE INVENTARIO" : tabla = "AdcNiv"
            etiqueta = "Artículos"
        End If
