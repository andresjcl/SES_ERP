...
namespace DaxInvLib
{
    class Datos
    {
        static internal void CargarMalla(DataGridView malla, string  Bodega,DateTime FechaFin,Boolean ordenAlfabetico)
        {
            ...
            string sSQL = "Select bod_codigo from emp_bod where emp_codigo = " + varbleComun.VarCom.codEmpresa;
            if (Bodega != "" && Bodega != "0")
            {
            ...
            using (SqlDataAdapter da = new SqlDataAdapter(sSQL, varbleComun.VarCom.strConxSyscod))
            {
