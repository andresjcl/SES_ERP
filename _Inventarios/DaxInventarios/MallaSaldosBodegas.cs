...
namespace DaxInvent
{
    class MallaSaldosBodegas
    {
        ...
        static internal void CargarMallafiltro(string codigo, string categoria, string clase, string grupo, DataGridView malla, string Bodega, DateTime FechaFin, Boolean ordenAlfabetico)
        {
            //string articuloini = "";
            string articuloini = codigo;

            //string articulofin = "";
            string articulofin = codigo;
            string TxtSuc = "";
            string SUMAR = "";
            string QueSaldo = " where tra_fecha <= '" + FechaFin.ToShortDateString() + "' ";

            ///AQUI CAMBIA DE 0 A TODAS LAS BODEGAS
            //string sSQL = "Select bod_codigo from emp_bod where emp_codigo = " + VarCom.codEmpresa;
            string sSQL = "Select Bod_codigo, bod_nombre from emp_bod where emp_codigo = " + datosEmpresa.codEmpresa;
            if (Bodega != "" && Bodega != "0")
            {
                sSQL += " and bod_codigo = '" + Bodega + "' ";
            }
            sSQL += " group by bod_codigo, bod_nombre ";
            ...
            sSQL = "SELECT AdcArt.Art_categor as CAT,AdcArt.Art_clase as CLA ,AdcArt.Art_nombre AS Descripción, R1.*, " + SUMAR + " AS TOTAL ";
            sSQL += " FROM AdcArt right OUTER JOIN";
            sSQL += " (SELECT Articulo, " + TxtSuc;
            sSQL += " FROM tmpSaldoBode AS R ";
            sSQL += " GROUP BY ARTICULO) R1 ON AdcArt.Art_codigo = R1.ARTICULO ";

            ///AQUI SE AGREGA SI HAY CATEGORIA, CLASE Y GRUPO

            if (categoria != "" && clase == "" && grupo == "")
            {
                sSQL += " WHERE AdcArt.Art_categor='" + categoria + "'";
            }
            if (categoria != "" && clase != "" && grupo == "")
            {
                sSQL += "WHERE AdcArt.Art_categor = '" + categoria + "'" + " AND AdcArt.Art_clase='" + clase + "'";
            }

            if (categoria == "" && clase != "" && grupo == "")
            {
                sSQL += "WHERE AdcArt.Art_clase='" + clase + "'";
            }

            if (categoria != "" && clase != "" && grupo != "")
            {
                sSQL += " WHERE AdcArt.Art_categor = '" + categoria + "'" + " AND AdcArt.Art_clase='" + clase + "'" + " AND AdcArt.Art_grupo='" + grupo + "'";
            }

            if (categoria == "" && clase == "" && grupo != "")
            {
                sSQL += " WHERE AdcArt.Art_grupo='" + grupo + "'";
            }

            if (categoria == "" && clase != "" && grupo != "")
            {
                sSQL += " WHERE AdcArt.Art_clase='" + clase + "'" + " And AdcArt.Art_grupo='" + grupo + "'";
            }

            if (categoria != "" && clase == "" && grupo != "")
            {
                sSQL += " WHERE AdcArt.Art_categor='" + categoria + "'" + " And AdcArt.Art_grupo='" + grupo + "'";
            }
            ...
            //{
            //    sSQL += " AND AdcArt.Art_codigo='" + codigo + "'";
            //}
            //sSQL += " group by bod_codigo ";
            ...
            using (SqlDataAdapter da = new SqlDataAdapter(sSQL,datosEmpresa.strConxAdcom))
            {
                ...
                {
                    MessageBox.Show("No existe registros para mostrar", "Mensaje Informativo", MessageBoxButtons.OK, MessageBoxIcon.Error);
