...
#Region "Malla"
    Private Sub FormatoMalla()
        With malla
            ...
            '.Columns("Niv_IdCta4").HeaderText = "Id.Contable-4"
    ...
    Private Sub llenarMalla(ByVal nivel As Integer, ByVal dest As String)
        ...
        Dim ssql As String = "select Niv_abrevia,Niv_nombre,Niv_IdCta,Niv_Idcta2,Niv_Idcta3,niv_grafico  from " & tabla & " where Niv_categor =" & nivel
    ...
    Private Sub malla_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles malla.KeyDown
        If e.KeyCode = Keys.Delete Then
            ...
            If PuedoEliminar(tb, cmp, CodCateg) = False Then
                MsgBox("Existen " & etiqueta & " con " & IIf(op = 1 Or op = 2, "esta ", "este ").ToString() & cboNivel.Text.ToLower & ". No se puede eliminar")
            Else
                If MsgBox("Confima que desea eliminar el registro? ", MsgBoxStyle.YesNo) = vbYes Then
...
#Region "Eliminar"
    ...
    Private Function PuedoEliminar(ByVal tabla As String, ByVal campo As String, ByVal cod As String) As Boolean
        ...
        If dat.Read Then
            If IsDBNull(dat("Sev_Codigo")) Then ban = True Else ban = False
        Else
            ban = True
        End If
        conectar.Close()
...
#Region "Imprimir"
    ...
    Private Sub imprimir()
        Dim ssql As String = "select * from adcNivServ"
        Dim imp As New FrmImpr()
        imp.Imprimir(ssql, "Reporte de Niveles de Servicios", "E")
    End Sub
#End Region
