...
{
    class MoverDatosClases
    {
        private void MoverDatosClase(ClassDoc.AdcDoc datADCDOC, FormFacPVT formPv)
        {
            //datADCDOC.Doc_sucursal = varbleComun.VarCom.suc;
            //datADCDOC.Doc_Bodega = formPv.cmbBodega.SelectedValue.ToString();
            //datADCDOC.Opc_documento = formPv.cmbDocumento.SelectedValue.ToString();
            //datADCDOC.Doc_numero = Convert.ToDecimal(formPv.txtnumero.Text);
            //datADCDOC.Doc_fecha = Convert.ToDateTime(formPv.txtfecha.Text);
            //datADCDOC.Doc_codper = formPv.codCliente;
