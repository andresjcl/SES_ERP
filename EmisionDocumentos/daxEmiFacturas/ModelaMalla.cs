...
namespace DctosEmi
{
    class ModelaMalla
	{
		...
		public void diseñarMallaInventario(ref System.Windows.Forms.DataGridView malla, ref sesSys.OpcDoc propedadesDocmto, daxAccs.propiedadesDaxAuto accesosLocalizados = null)
		{
			if (malla == null) return;
			...
			// =============================
			// HEADERS - SOLO INVENTARIO (SIN DESCUENTOS NI IVA)
			// =============================
			malla.Columns["tra_codigo"].HeaderText = "Código";
			malla.Columns["tra_nombre"].HeaderText = "Descripción";
			malla.Columns["tra_cantidad"].HeaderText = "Cantidad";
			malla.Columns["Tra_precuni"].HeaderText = "Costo Unitario";
			malla.Columns["Tra_prectot"].HeaderText = "Costo Total";
			malla.Columns["tra_medida"].HeaderText = "Med.";
			malla.Columns["TRA_NUMLINEA"].HeaderText = "Nro";

			// =============================
			// VISIBILIDAD - SOLO COLUMNAS ESENCIALES
			// =============================
			malla.Columns["tra_codigo"].Visible = true;
			malla.Columns["tra_nombre"].Visible = true;
			malla.Columns["tra_cantidad"].Visible = true;
			...
			// ❌ OCULTAR COLUMNAS DE DESCUENTO
			if (malla.Columns.Contains("tra_porcendes"))
			{
				malla.Columns["tra_porcendes"].Visible = false;
				malla.Columns["tra_porcendes"].ReadOnly = true;
			}

			if (malla.Columns.Contains("tra_valordes"))
			{
				malla.Columns["tra_valordes"].Visible = false;
				malla.Columns["tra_valordes"].ReadOnly = true;
			}

			// ❌ OCULTAR COLUMNAS DE IVA
			if (malla.Columns.Contains("Tra_porceniva"))
			{
				malla.Columns["Tra_porceniva"].Visible = false;
				malla.Columns["Tra_porceniva"].ReadOnly = true;
			}

			if (malla.Columns.Contains("Tra_valoriva"))
			{
			...
			if (malla.Columns.Contains("tra_SnIva"))
			{
				...
				malla.Columns["tra_SnIva"].ReadOnly = true;
			}

			// ✅ COLUMNAS OPCIONALES DE INVENTARIO
			if (datosAuxiliares.tieneDatoDetalle("Existencia", propedadesDocmto))
			{
				malla.Columns["Existencia"].HeaderText = "Stock Actual";
				malla.Columns["Existencia"].Visible = true;
				malla.Columns["Existencia"].ReadOnly = true;
			}

			if (datosAuxiliares.tieneDatoDetalle("BodegaMovimiento", propedadesDocmto))
			{
				malla.Columns["Doc_Bodega"].HeaderText = "Bodega";
				malla.Columns["Doc_Bodega"].Visible = true;
			}
			...
			malla.Columns["Tra_prectot"].ReadOnly = true;
			malla.Columns["TRA_NUMLINEA"].ReadOnly = true;
			malla.Columns["tra_nombre"].ReadOnly = true; // La descripción viene del producto

			// =============================
			// FORMATOS
			// =============================
			malla.Columns["Tra_precuni"].DefaultCellStyle.Format = $"N{digPreciosV}";
			malla.Columns["Tra_prectot"].DefaultCellStyle.Format = $"N{digPreciosV}";
			malla.Columns["tra_cantidad"].DefaultCellStyle.Format = "N2";
			...
			malla.Columns["Tra_precuni"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
			malla.Columns["Tra_prectot"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
			...
			if (accesosLocalizados == null || accesosLocalizados.sinRegistro)
				return;

			// SOLO PERMISOS PARA INVENTARIO (sin descuentos)
			if (malla.Columns.Contains("Tra_precuni"))
				malla.Columns["Tra_precuni"].ReadOnly = !accesosLocalizados.PrecioUnitario;			
		}
