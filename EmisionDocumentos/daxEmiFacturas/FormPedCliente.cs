...
namespace DctosEmi
{
	public partial class FormPedCliente : Form
	{
...
//        daxContaDoc.AsientosContables asientosContables = new daxContaDoc.AsientosContables();
		...
		public FormPedCliente(string clasdef, string tipdef, bool nuevo = false, Boolean esConsulta = false, Boolean generarFactura = false, Boolean desdeEstdoCta = false, idDocumento idDocViene = null, string sisMedico = "")
		{
			InitializeComponent();
		...
		private void LlenarCombos()
		{
			...
			{
				mensajesDocumento.Text ="NO EXISTEN DOCUMENTOS DEFINIDOS PARA PROFORMAS O PEDIDOS CLIENTE \n(Registrar en menú Documentos/Definir-Documentos)";
				SinDocumentos = true;
				return;
			}
			try
...
//                this.malla.CellValueChanged += new System.Windows.Forms.DataGridViewCellEventHandler(this.malla_CellValueChanged);
		...
		private void totalizar()
		{
			...
			if (malla.CurrentRow != null &&
				malla.CurrentRow.Cells["Tra_porceniva"] != null &&
				malla.CurrentRow.Cells["Tra_porceniva"].Value != null &&
				!string.IsNullOrWhiteSpace(malla.CurrentRow.Cells["Tra_porceniva"].Value.ToString()))
			{
				ivaM = Convert.ToDecimal(malla.CurrentRow.Cells["Tra_porceniva"].Value.ToString());
			}
			else
			{
				ivaM = 0;
				// Puedes registrar el error, asignar un valor por defecto o lanzar una advertencia
				//MessageBox.Show("No se puede obtener el valor de IVA porque la celda está vacía o no hay fila seleccionada.", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning);
			}
...
//End Sub
		...
		private void registraOpciones()
		{
			...
			if (cmbVendedor.SelectedValue != null ) AuditSis.registrar.registraPreferencia(datosEmpresa.strConxAdcom, datosEmpresa.codEmpresa.ToString(),Environment.MachineName, datosEmpresa.usr, "ADX", datosEmpresa.suc, "Facturacion", "Vendedor", cmbVendedor.SelectedValue.ToString());
		}
