...
namespace DctosEmi
{
	public partial class FormIngImportacion : Form
	{
        ...
        string claseDocDefault = "IMP";
		string tipoDocDefault = "IMP";
		decimal ivaM = 0;
		...
		string memVendedor = "";
		string memBodega = "";
		public FormIngImportacion(string clasdef, string tipdef, bool nuevo = false, Boolean esConsulta = false, Boolean generarFactura = false, Boolean desdeEstdoCta = false, idDocumento idDocViene = null, string sisMedico = "")
		{
			InitializeComponent();
                    ...
                    case "TRA_CODIGO":
                        if (dato != ".")
                        {
                            ...
                            else if (keyData == Keys.F3)
                            {
								...
								if (dato != "") mallaArticulo.CargarServicios(dato, ref malla, 0 , valoresPredefinidosEmpresa.nroDigitosEnPrecios, Convert.ToDateTime(txtfecha.Text), ref propiedadesDoc);
							}
...
//			this.malla.CellValueChanged -= new System.Windows.Forms.DataGridViewCellEventHandler(this.malla_CellValueChanged);
			...
			//	if (ivaM == 0) ; //claseImpuestos.cargar(datosEmpresa.strConxIvaret, txtfecha.Value);
			//}
			totalesDocumento = new DctosEmi.docTotals();
