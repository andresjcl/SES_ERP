...
namespace DctosEmi
{
	public partial class FormProforma: Form
	{
		...
		internal AdcDocPro datADCDOCPRO;
...
//        daxContaDoc.AsientosContables asientosContables = new daxContaDoc.AsientosContables();
        string claseDocDefault = "PRC";
		string tipoDocDefault = "PRC";
		decimal ivaM = 0;
		...
		Boolean SinDocumentos = false;

		public FormProforma(string clasdef, string tipdef, bool nuevo = false, Boolean esConsulta = false, Boolean generarFactura = false, Boolean desdeEstdoCta = false, idDocumento idDocViene = null, string sisMedico = "")
		{
			InitializeComponent();
			...
			if (idDocViene.idClave > 0 && esConsulta)
			{
				...
				// Asegurar que el cmbDocumento esté cargado antes de asignar valor
				...
				{
					// Guardar el valor para después
					memTipoDoc = idDocViene.Tipo;
				}
			}
		...
		private void formFacPv_Load(object sender, EventArgs e)
		{
			...
			{
				// Asegurar que el tipo de documento esté cargado
				if (string.IsNullOrEmpty(idDocumentoActual.Tipo) && !string.IsNullOrEmpty(memTipoDoc))
				{
					idDocumentoActual.Tipo = memTipoDoc;
				}
				cargarDatosProforma(idDocumentoActual.Sucursal, idDocumentoActual.Tipo, idDocumentoActual.idClave);
			}
		...
		private void LlenarCombos()
		{
			...
			if (cmbDocumento.Items.Count == 0)
			{
				mensajesDocumento.Text ="NO EXISTEN DOCUMENTOS DEFINIDOS PARA PROFORMAS CLIENTE \n(Registrar en menú Documentos/Definir-Documentos)";
				SinDocumentos = true;
				return;
			}
			//try
			...
			try
			{
				if (!string.IsNullOrEmpty(memTipoDoc))
				{
					// Verificar que el valor existe en el combo
					bool existeValor = false;
					foreach (DataRowView item in cmbDocumento.Items)
					{
						if (item.Row["Opc_documento"].ToString() == memTipoDoc)
						{
							existeValor = true;
							break;
						}
			...
			//if (cmbDocumento.SelectedIndex == -1) cmbDocumento.SelectedIndex = 0;
			...
			if (!string.IsNullOrEmpty(memBodega))
			{
			...
			if (cmbVendedor.Items.Count > 0)
			{
				if (!string.IsNullOrEmpty(memVendedor))
				{
		...
		private void CargarPredefinidosDocumento()
		{
			// Asegurar que idDocumentoActual.Tipo tenga valor
			if (string.IsNullOrEmpty(idDocumentoActual.Tipo))
			{
				if (cmbDocumento.SelectedValue != null)
				{
					idDocumentoActual.Tipo = cmbDocumento.SelectedValue.ToString();
		...
		//{
		//	if (SinDocumentos) operacionEnCurso = sinOperacion;
		...
		//	panel1.Enabled = (!inicio || SinDocumentos);
		...
		//		if (datADCDOCPRO.Doc_Estado == 1) btnElimina.Enabled = (datosEmpresa.usr.ToUpper() == "ADMINISTRADOR");
		//	}
		...
		private void registrarAccesosLocalizadosDocumento()
		{
			...
			txtNroID.Enabled = false;// txtnumero.Enabled;
		...
		private void LlenarComboDocReferencia()
		{
			...
			{
				cmbx.DaxCombosDoc("", propiedadesDoc.TipoSoporteObligatorio, false, datosEmpresa.strConxAdcom, ref cmbDocumentoSustento);
//				Ssql = "SELECT opc_nombre, opc_documento, opc_tipo FROM adcopc WHERE OPC_DOCUMENTO = '" + propiedadesDoc.TipoSoporteObligatorio + "' order by opc_documento ";
			...
			{
				cmbx.DaxCombosDoc("PECPRC", "PEC", false, datosEmpresa.strConxAdcom, ref cmbDocumentoSustento);
//				Ssql = "SELECT opc_nombre, opc_documento, opc_tipo FROM adcopc  WHERE OPC_DOCUMENTO > '' order by opc_documento ";
				...
				//cmbDocumentoSustento.DataSource = SqlDatos.leerTablaAdcom(Ssql);
		...
		//      private Boolean cargarDatosProforma(string SUC, string TIPO, double IDCLAVE)
		//{
		//	Boolean resp = false;
		//	if (IDCLAVE != 0)
		//	{
		//		datADCDOCPRO = new AdcDocPro(datosEmpresa.strConxAdcom);
		...
		//			cargarFormaDePago(idDocumentoActual, "ADCPAGPRO");
		//			totalizar();
		...
		private Boolean cargarDatosProforma(string SUC, string TIPO, double IDCLAVE)
		{
			...
			// Validar que TIPO no sea null o vacío
			if (string.IsNullOrEmpty(TIPO))
			{
				MessageBox.Show("Error: Tipo de documento no definido", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
				return false;
			}
			...
			if (IDCLAVE != 0)
			{
				...
				if (datADCDOCPRO != null)
				{
					...
					if (Convert.ToInt32(datADCDOCPRO.Doc_Estado) == 0)
						mensajesDocumento.Text = "DOCUMENTO ANULADO : " + datADCDOCPRO.MotivoAnulacion;
		...
		private void cargarDetalleArticulos(string suc, string tip, double idClave, string tablatra)
		{
			...
			{
				MessageBox.Show("Error: Tipo de documento no especificado para cargar detalle", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
				return;
			}
		...
		private void cargarDetalleArticulosConsolidacion(string listaDocumentos)
		{
			...
			dtDetalleDocumento = utilBasDatos.utilBasDatos.leerTablas(dcut.armarSqlLecturaTraFacConsolida("ADCTRAPRO", listaDocumentos), datosEmpresa.strConxAdcom);
			dcut = null;
		...
		private void InicializarMalla()
		{
			...
			dtDetalleDocumento = utilBasDatos.utilBasDatos.leerTablas(dcut.armarSqlLecturaTransFacturas("ADCTRAPRO", datosEmpresa.suc, idDocumentoActual.Tipo, idDocumentoActual.idClave), datosEmpresa.strConxAdcom);
			//DctosEmi.armarConsTra dcut = new DctosEmi.armarConsTra();
			//dtDetalleDocumento = utilBasDatos.utilBasDatos.leerTablas(dcut.armarSqlLecturaTraFacPv(propiedadesDoc, "ADCTRAPRO", "", "", 0), datosEmpresa.strConxAdcom);
			if (dtDetalleDocumento == null) return;
			malla.DataSource = dtDetalleDocumento;
		...
		private void btnCopia_Click(object sender, EventArgs e)
		{
			...
			progBus.IniciaBusqueda("PRCPRPFACFAPPECPEPNCPNDPNCCNDCREM", "", "", queFecha, ref SUC, ref TIP, ref Numero, ref Idclave, false, "", "", "","ADCDOCPRO");
			if (Idclave != 0)
			{
		...
		private void btnConsolida_Click(object sender, EventArgs e)
		{
			...
			{
				Lista = progBus.IniciaConsolidacion(DctosEmi.ConsolidaDoc.tiposDoctsConsolidaSql(datosEmpresa.strConxAdcom), codCliente, "", queFecha, ref SUC, ref TIP, ref Numero, ref Idclave, false, "", "", "", "ADCDOCPRO")
			};
		...
		private void BtnAbre_Click(object sender, EventArgs e)
		{
			...
			if (idDocumentoActual.idClave != 0) cargarDatosProforma(idDocumentoActual.Sucursal, idDocumentoActual.Tipo, idDocumentoActual.idClave);
		}
		...
		private void iniciarNuevoDocumento()
		{
			...
			idDocumentoActual = new idDocumento
			{
				...
				Serie = txtNroID.Text,
				Sucursal = datosEmpresa.suc,
				Tipo = idDocumentoActual.Tipo  // Usar el valor ya obtenido
			};
		...
		private void btnRegistra_Click(object sender, EventArgs e)
		{
			if (validarDocumento() == true)
			{
				...
				{
					EnviarAimpresora.imprimirDocumentoDirectamente(datADCDOCPRO , accesosLocalizados, idDocumentoActual);
					limpiarDatos();
				}
			}
		}
		private void btnAnula_Click(object sender, EventArgs e)
		{
			...
			if (classAnular.anularDocumento(datosEmpresa.strConxAdcom, datosEmpresa.strConxSyscod, idDocumentoActual, datosEmpresa.usr, esDeLiquidacion, datosEmpresa.nomEmpresa, datosEmpresa.codEmpresa.ToString(), edTotal.Text, "ADCDOCPRO", propiedadesDoc.ComandoExterno)) limpiarDatos();
			classAnular = null;
		}
		private void btnElimina_Click(object sender, EventArgs e)
		{
			DctosEmi.anulaElimina classAnular = new DctosEmi.anulaElimina();
			if (classAnular.eliminarDocumento(datosEmpresa.strConxAdcom, datosEmpresa.strConxSyscod, idDocumentoActual, datosEmpresa.usr, esDeLiquidacion, datosEmpresa.nomEmpresa, datosEmpresa.codEmpresa.ToString(), edTotal.Text, "ADCDOCPRO", propiedadesDoc.ComandoExterno)) limpiarDatos();
			classAnular = null;
		}
		...
		private void cmbDocumento_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			string nuevoTipo = cmbDocumento.SelectedValue.ToString();

			if (idDocumentoActual.Tipo == nuevoTipo) return;

			idDocumentoActual.Tipo = nuevoTipo;
			CargarPredefinidosDocumento();
		}
		...
		private void verificaNroDocumentoDigitado()
		{
			...
			impresionVerificacion.verificarExistenciaDocumento(ref idDocumentoActual, datosEmpresa.strConxAdcom,false, "ADCDOCPRO",txtcedula.Text );
			if (idDocumentoActual.idClave > 0) cargarDatosProforma(idDocumentoActual.Sucursal, idDocumentoActual.Tipo, idDocumentoActual.idClave);
		}
		...
		{
			if (accesosLocalizados.NroImpresiones > 0 && accesosLocalizados.NroImpresiones <= datADCDOCPRO.Doc_Adicional1)
			...
			datADCDOCPRO.Doc_Adicional1 = imp;
		}
		...
		private void copiarDocumento(idDocumento idDocCopiar, Boolean esGenerar = false)
		{
			...
			copia.CopiarElDocumento(idDocCopiar,opcp.tablaDatosDoc,ref datADCDOCPRO , ref dtDetalleDocumento);
			if (datADCDOCPRO != null && datADCDOCPRO.Doc_codper.Length > 0)
            {
                datADCDOCPRO.IdClaveDoc = 0;
                datADCDOCPRO.Doc_numero = Convert.ToDecimal(txtnumero.Text);
				datADCDOCPRO.Doc_fecha = txtfecha.Value;
                if (esGenerar == false)
                {
...
//                this.malla.CellValueChanged += new System.Windows.Forms.DataGridViewCellEventHandler(this.malla_CellValueChanged);
		...
		private Boolean validarDocumento()
		{
            ...
            DatosFacturacion.moverDatosAclase(this,datADCDOCPRO);
			return true;
		}
		...
		//                  idDocumentoActual.Sucursal = datADCDOCPRO.Doc_sucursal;
		//                  idDocumentoActual.Tipo = datADCDOCPRO.Opc_documento;
		//			txtnumero.Text = datADCDOCPRO.Doc_numero.ToString();
		//			actualizaDatosPagos();
		...
		//			clasePagos.guardarPagosDocumento("ADCPAGPRO");
		//                  //ClaveElectronica.actualizarClaveElectronica(datADCDOC);
		//                  AuditSis.registrar.registraEventoDoc(datosEmpresa.strConIniSis, datosEmpresa.codEmpresa.ToString(), datosEmpresa.usr, "MDD", Environment.MachineName, AuditSis.registrar.EvntCrear, idDocumentoActual.Sucursal, idDocumentoActual.Tipo, idDocumentoActual.numero.ToString(), datADCDOCPRO.Doc_valor.ToString());
		//              }
		//              else
		//              {
		...
		//                  actualizaDatosPagos();
		//                  clasePagos.guardarPagosDocumento("ADCPAGPRO");
		//                  AuditSis.registrar.registraEventoDoc(datosEmpresa.strConIniSis, datosEmpresa.codEmpresa.ToString(), datosEmpresa.usr, "MDD", Environment.MachineName, AuditSis.registrar.EvntModifica, idDocumentoActual.Sucursal, idDocumentoActual.Tipo, idDocumentoActual.numero.ToString(), datADCDOCPRO.Doc_valor.ToString());
		//              }
		//          }
		//          //catch (Exception ee)
		//          {
		...
		private Boolean grabarDocumento()
		{
			...
			// Validar que idDocumentoActual.Tipo tenga valor
			if (string.IsNullOrEmpty(idDocumentoActual.Tipo))
			{
				MessageBox.Show("Error: Tipo de documento no definido", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
				return false;
			...
			{
				res = "ERR " + ex.Message;
				MessageBox.Show("Error al grabar documento: " + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
			}
		...
		//{
		//	grabMallTra.grabarMallaAdctraPro(malla, datADCDOCPRO,datosEmpresa.strConxAdcom);
		//}
		...
		private void grabarAdctra()
		{
			...
			if (string.IsNullOrEmpty(idDocumentoActual.Tipo))
			{
				MessageBox.Show("Error: No se puede grabar transacción sin tipo de documento", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
				return;
			}
		...
		private void totalizar()
		{
			try
			{
				// Verificar si la malla tiene datos
				if (malla.Rows.Count == 0)
				{
					// Inicializar totalesDocumento si es null
					if (totalesDocumento == null)
						totalesDocumento = new DctosEmi.docTotals();
				...
				// Obtener el valor del IVA de manera segura
				ivaM = ObtenerPorcentajeIVASeguro();

				// Actualizar impuestos si es necesario
				if (claseImpuestos.cambiadoManual == false)
				{
					if (claseImpuestos.impstoPorcentaje1 == 0 ||
						claseImpuestos.impstoFechaIni1 > txtfecha.Value ||
						claseImpuestos.impstoFechaFin1 < txtfecha.Value)
					{
						claseImpuestos.cargar(datosEmpresa.strConxIvaret, txtfecha.Value);
					}
				}

				// Crear nueva instancia de totales
				totalesDocumento = new DctosEmi.docTotals();

				// Calcular totales
				decimal total = totalesDocumento.totalizar(
					malla,
					ivaM,
					claseDescuentos,
					clasePagos,
			...
			catch (Exception ex)
			{
				...
				// MessageBox.Show("Error en totalizar: " + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
			}
		}

		// Método auxiliar para obtener el porcentaje de IVA de manera segura
		private decimal ObtenerPorcentajeIVASeguro()
		{
			try
			{
				// Verificar si hay fila actual seleccionada
				if (malla.CurrentRow == null)
				{
					// Si no hay fila actual, buscar la primera fila con datos
					foreach (DataGridViewRow row in malla.Rows)
					{
						if (row.Cells["Tra_porceniva"].Value != null &&
							!string.IsNullOrEmpty(row.Cells["Tra_porceniva"].Value.ToString()))
						{
							string valorIva = row.Cells["Tra_porceniva"].Value.ToString();
							if (decimal.TryParse(valorIva, out decimal ivaDecimal))
							{
								return ivaDecimal;
							}
						}
					...
					// Si no se encontró ningún valor, usar el de claseImpuestos
					return Convert.ToDecimal(claseImpuestos.impstoPorcentaje1);
				}
				else
				{
					// Usar la fila actual
					if (malla.CurrentRow.Cells["Tra_porceniva"].Value != null)
					{
						string valorIva = malla.CurrentRow.Cells["Tra_porceniva"].Value.ToString();
						if (decimal.TryParse(valorIva, out decimal ivaDecimal))
						{
							return ivaDecimal;
						}
					...
					// Si la fila actual no tiene valor, buscar en otras filas
					foreach (DataGridViewRow row in malla.Rows)
					{
						if (row != malla.CurrentRow &&
							row.Cells["Tra_porceniva"].Value != null &&
							!string.IsNullOrEmpty(row.Cells["Tra_porceniva"].Value.ToString()))
						{
							string valorIva = row.Cells["Tra_porceniva"].Value.ToString();
							if (decimal.TryParse(valorIva, out decimal ivaDecimal))
		...
		// Método para establecer todos los totales en cero (para cuando no hay datos o hay error)
		private void establecerTotalesEnCero()
		{
			try
			{
				// Asegurar que totalesDocumento existe
				if (totalesDocumento == null)
					totalesDocumento = new DctosEmi.docTotals();

				// Establecer propiedades en cero
				totalesDocumento.totdesciva = 0;
				totalesDocumento.totdessiva = 0;
				totalesDocumento.TotIva = 0;
				totalesDocumento.Subtotalciva = 0;
				totalesDocumento.SubTotalSIva = 0;
				totalesDocumento.TotCiva = 0;
				totalesDocumento.TotSiva = 0;
				totalesDocumento.TotDescuentos = 0;
				totalesDocumento.TotVta = 0;

				// Llamar a tu método presentarTotales existente
				presentarTotales();
			...
			catch (Exception ex)
			{
				...
				labTotIva.Text = "0.00";
				labTotPorcIva.Text = (claseImpuestos.impstoPorcentaje1).ToString("0.00") + "% IVA";
				labTotSubConIva.Text = "0.00";
				labTotSubSinIva.Text = "0.00";
				labTotVtaConIva.Text = "0.00";
				labTotVtaSinIva.Text = "0.00";
				labTotalDescuento.Text = "0.00";
				labTotalVenta.Text = "0.00";
		...
		// Mantener tu método presentarTotales original sin cambios
		private void presentarTotales()
		{
			try
			{
				...
				// Asegurar que totalesDocumento no sea null
				if (totalesDocumento == null)
				{
					totalesDocumento = new DctosEmi.docTotals();
				}
...
//End Sub
        ...
        {
			docMallaArticulo docMalla = new docMallaArticulo();
			WaitMensaje.WMensaje.verMensaje("Agrupando artículos iguales");
			docMalla.AcumularLineasMalla(ref malla, "Tra_codigo", "Tra_Cantidad","Tra_Precuni");
			totalizar();
