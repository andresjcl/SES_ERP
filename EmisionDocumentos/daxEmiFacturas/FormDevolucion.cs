...
namespace DctosEmi
{
	public partial class FormDevolucion: Form
	{
        ...
        string claseDocDefault = "DEV";
		string tipoDocDefault = "DEV";
		...
		public FormDevolucion(string clasdef, string tipdef, bool nuevo = false, Boolean esConsulta = false, Boolean generarFactura = false, Boolean desdeEstdoCta = false, idDocumento idDocViene = null, string sisMedico = "")
		{
			InitializeComponent();
        ...
        private void LlenarComboDocReferencia()
		{
			...
			{
				Ssql = "SELECT opc_nombre, opc_documento, opc_tipo FROM adcopc  WHERE OPC_TIPO in  ('FAC','NDC','ING') order by opc_documento ";
...
//End Sub
		...
		private void registraOpciones()
		{
			...
			if (cmbVendedor.SelectedValue != null ) AuditSis.registrar.registraPreferencia(datosEmpresa.strConxAdcom, Environment.MachineName, datosEmpresa.codEmpresa.ToString(), datosEmpresa.usr, datosEmpresa.sistema, datosEmpresa.suc, "Facturacion", "Vendedor", cmbVendedor.SelectedValue.ToString());
		}
...
//				listaDocAplicados.ListaDocAplican.Clear();
        ...
        private void btnBuscaDocumentoSoporte_Click(object sender, EventArgs e)
        {
			...
			DateTime queFecha = docUtils.DaxNow();
			progBus.IniciaBusqueda("FACINGNDC", "", cmbDocumentoSustento.SelectedValue.ToString(), queFecha, ref idDocumentoSoporte.Sucursal, ref idDocumentoSoporte.Tipo, ref idDocumentoSoporte.numero, ref idDocumentoSoporte.idClave, false, cmbDocumentoSustento.SelectedValue.ToString(), "", "", "ADCDOC");
