...
namespace adcDocumentos
{
    public class BusquedasDetalleDocumentos
    {
        public static void BuscarValor(DataGridViewCell cell,string nombreCol)
        {
            ...
            {
                case "DOC_BODEGA":
                    comando = "select Bod_codigo as Codigo,Bod_nombre as Descripcion from Daxsys.dbo.Emp_Bod where emp_codigo = " + varbleComun.VarCom.codEmpresa;
                    break;
                case "TRA_EMPLEADO":
            ...
            cell.Value = busca.Buscar(varbleComun.VarCom.strConxAdcom, comando, "Codigo", "Descripcion", "", titulo);
            busca.Dispose();
        }
    }
}
