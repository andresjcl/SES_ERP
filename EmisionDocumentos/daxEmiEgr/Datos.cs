...
namespace adcDocumentos
{
    internal class DatosDocBancos
    {
        ...
        //internal void BuscarCtaBanco(string codigo,string NroCta, string nombre)
        //{

        //}
        internal Boolean CargarConceptoDoc(string dato,  DataGridViewRow row, PrySysp13.OpcDoc opcDoc,string QueBeneficiario,string BancoFin)
        {
            if (row == null) return false;
            if (dato == "") return false;

            daxConta.reglCtb dxCtb = new daxConta.reglCtb();
            ClassDoc.Servicios OpServicio = new ClassDoc.Servicios(SysEmpDatt.datosEmpresa.strConxAdcom);
        ...
        {
            formulario.datADCDOC.Doc_sucursal = SysEmpDatt.datosEmpresa.sucursal;
