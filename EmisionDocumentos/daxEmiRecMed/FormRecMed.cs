...
{
	public partial class FormRecMed : Form
	{
		...
		string claseDocDefault = "REC";
		string tipoDocDefault = "REC";
		...
		DaxMedic.DatosEmitirReceta datosRecetaMedica = new DaxMedic.DatosEmitirReceta();
		string memTipoDoc = "";
		string memVendedor = "";
		...
		public FormRecMed(string clasdef, string tipdef, bool nuevo = false, Boolean esConsulta = false, Boolean generarFactura = false, Boolean desdeEstdoCta = false, idDocumento idDocViene = null, DaxMedic.DatosEmitirReceta datosReceta= null)
		{
			InitializeComponent();
			CargarValoresIniciales();
			if (datosReceta != null) datosRecetaMedica = datosReceta;			
		...
		{
			int nroConceptos = 0;
			try { nroConceptos = datosRecetaMedica.ConceptoCodigo.Length; }catch { };
        ...
        {
            if (LlamadodesdeCitas && datosRecetaMedica.IdclaveCitaMedica > 0)
            {
                if (saleSinFacturar)
                {
                    bool borrar = true;
                    ...
                    if (borrar)
                    {
                        string ssql = "Delete medcitas where idClave = " + datosRecetaMedica.IdclaveCitaMedica.ToString();
		...
		private void FacturarAtencionMedicaExterna()
		{
			...
			//cmbDocumento.SelectedValue = datosRecetaMedica.TipoDoc;
			//txtcedula.Text = datosRecetaMedica.CirucPaciente;
			//codCliente = datosRecetaMedica.CodigoPaciente;
			//TurnoMedico = Convert.ToInt16(datosRecetaMedica.NroTurno);
			//cmbVendedor.SelectedValue = datosRecetaMedica.CodigoDoctor;
			//if (txtcedula.Text.Length > 0) ingresaCodigoClienteDirecto();
			//SesMedic.ImportarFacturaMedica imp = new SesMedic.ImportarFacturaMedica();
			//imp.ImportarServiciosMedicos(dtDetalleDocumento, malla, datosRecetaMedica);
			////totalizar();
			//this.malla.CellValueChanged += new System.Windows.Forms.DataGridViewCellEventHandler(this.malla_CellValueChanged);
		}
	...
	private void CargarPredefinidosDocumento()
		...
		private Boolean cargarDatosFactura(string SUC, string TIPO, double IDCLAVE)
		{
			...
			if (IDCLAVE != 0)
			{
				datADCDOC = new AdcDocRec(datosEmpresa.strConxAdcom);
				datADCDOC = AdcDocRec.Buscar(" doc_sucursal = '" + SUC + "' and opc_documento ='" + TIPO + "' and idclavedoc = " + IDCLAVE.ToString());
				if (datADCDOC != null)
				{
		...
		private void limpiarDatos()
		{
            ...
            datosRecetaMedica = new DaxMedic.DatosEmitirReceta();
		...
		private void iniciarNuevoDocumento()
		{
            ...
            string bod = "";		
						txtfecha.Value = docUtils.DaxNow();
			ClassRec.controlNumeracion cnum = new controlNumeracion();
		...
		{
			//if (datosRecetaMedica.nropedidoLab == "CREAR") CrearPedidoLaboratorio(); else ActualizarPedidoLaboratorio();
			//if (datosRecetaMedica.nroPedidoImg == "CREAR") CrearPedidoImagenologia(); else ActualizarPedidoImagenologia();
			string ssql = "";
			if (datosRecetaMedica.IdclaveCitaMedica > 0)
			{
				ssql = "Update medcitas set factura = '" + datosEmpresa.sucursal + "-" + idDocumentoActual.Serie + "-" + idDocumentoActual.numero.ToString() + "'";
				ssql += " where idclave = " + datosRecetaMedica.IdclaveCitaMedica.ToString();
				SqlDatos.ejecutarComandoAdcom(ssql);
			}
			ssql = "MedGenPedLab '" + idDocumentoActual.idClave.ToString() + "','" + idDocumentoActual.Serie +" - "+ idDocumentoActual.numero.ToString() + "','" + idDocumentoActual.Sucursal.ToString() + "','" + idDocumentoActual.Tipo.ToString() + "','" + datosRecetaMedica.CodigoPaciente.Trim()  + "','" + datosRecetaMedica.DrPedidoLab + "'";
			SqlDatos.ejecutarComandoAdcom(ssql);
		...
		private void verificaNroDocumentoDigitado()
		{
			...
			// impresionVerificacion.verificarExistenciaDocumento(ref idDocumentoActual, datosEmpresa.strConxAdcom,false,"ADCDOCREC","");
			//if (idDocumentoActual.idClave > 0) cargarDatosFactura(idDocumentoActual.Sucursal ,idDocumentoActual.Tipo, idDocumentoActual.idClave);
		}
	   ...
	   {         
		...
		private Boolean funcionesEspeciales(ref Keys keyData, DataGridView malla)
		{
			...
			if (nombreCelda == "TRA_PRECUNI" && keyData >= Keys.F2 && keyData <= Keys.F6)
			{
				...
				Int32 quePrecio = 0;
				cell.Value =  cargarPrecios.CargarPrecioVta (Convert.ToInt16(keyData), malla.CurrentRow.Cells["tra_Codigo"].Value.ToString(), 0,ref quePrecio,"", row.Cells["tra_Medida"].Value.ToString(),"", valoresPredefinidosEmpresa.nroDigitosEnPrecios);
//				totalizar();
			}
     ...
     //               else if (keyData == Keys.F3)
		...
		private Boolean validarDocumento ()
		{
			...
			if (ValidacionDocumentos.ValidacionGeneral.ControlValidacionFacCli(ref malla, ref propiedadesDoc, clasref, txtfecha.Text, datosEmpresa.suc, datosEmpresa.strConxAdcom, datosEmpresa.strConxSyscod, idDocumentoActual.idClave, "", txtnumero.Text, entregasPendientes, datosEmpresa.suc, docsustento, "") == false) return false;
		...
		//			string tipDoc = cmbDocumento.SelectedValue.ToString();
					...
					registraEvntos.registrar.registraEventoDoc(datosEmpresa.strConxDaxsys, datosEmpresa.codEmpresa.ToString(), datosEmpresa.usr, "MDD",Environment.MachineName,registraEvntos.registrar.EvntCrear,idDocumentoActual.Sucursal,idDocumentoActual.Tipo,idDocumentoActual.numero.ToString(),0.ToString());
				}
				else
				{
					...
					//actualizaDatosPagos();
					//clasePagos.guardarPagosDocumento("ADCPAG");
					registraEvntos.registrar.registraEventoDoc(datosEmpresa.strConxDaxsys, datosEmpresa.codEmpresa.ToString(), datosEmpresa.usr, "MDD", Environment.MachineName, registraEvntos.registrar.EvntModifica, idDocumentoActual.Sucursal, idDocumentoActual.Tipo, idDocumentoActual.numero.ToString(), 0.ToString());
				}
			}
			catch (Exception ee)
			{
		...
		private void btnCierra_Click_1(object sender, EventArgs e)
		{
