...
public class AdcArt
{
    ...
    private System.Boolean _Art_sniva=false;
    ...
    public string detalleLargo = "";

    public string talla = "";
    public string color = "";
    public string serie = "";
    public double tiket = 0;

    public string idContable="";
    public string idContable2="";
    public string idContable3="";
    public string idContable4="";
    ...
    private static string cadenaConexion = @"Data Source=192.168.131.5; Initial Catalog=bdadcomdx14acerconsq; user id=sa; password=123qweASDZXC";
    ...
    private static AdcArt row2AdcArt(DataRow r){
        ...
        try{
            ...
            //       o la fecha de hoy:
            oAdcArt.Art_fecucom = DateTime.Now;
        }
        ...
        try{
            ...
            oAdcArt.Art_FechaCrea = DateTime.Now;
        }
        try{
    ...
    public static DataTable Tabla(string sel){
        // devuelve una tabla con los datos de la consulta
        SqlDataAdapter da;
    ...
    public static AdcArt Buscar(string sWhere){
        ...
        if(dt.Rows.Count > 0)
        {
            ...
            cargarDatosAlternos(oAdcArt);            
        }
        return oAdcArt;
    }
    private static void cargarDatosAlternos(AdcArt Art)
    { 
        string ssql=" SELECT * FROM  AdcTik where TIK_ARTCODI = '" + Art.Art_codigo + "'";
        DataTable dt = Tabla(ssql);
        if (dt.Rows.Count > 0) 
        {
            Art.color = dt.Rows [0]["tik_color"].ToString();
            Art.talla = dt.Rows [0]["tik_talla"].ToString();
            Art.serie = dt.Rows [0]["tik_serie"].ToString();
            Art.tiket = Convert.ToDouble("0" + dt.Rows [0]["tik_numero"].ToString ());
        }

        ssql="Select * from adcniv where niv_categor = 1 and niv_abrevia = '" + Art._Art_categor + "'" + Art.Art_codigo + "'";
        dt=Tabla(ssql);
        if (dt.Rows.Count > 0)
        {
            Art.idContable = dt.Rows[0]["Niv_IdCta"].ToString();
            Art.idContable2 = dt.Rows[0]["Niv_IdCta2"].ToString();
            Art.idContable3 = dt.Rows[0]["Niv_IdCta3"].ToString();
            Art.idContable4 = dt.Rows[0]["Niv_IdCta4"].ToString();
        }
    }
