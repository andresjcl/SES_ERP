...
namespace ClassDoc
{
    public class AdcDoc
    {
        ...
        public System.String TipoTransporte
        {
            get
            {
                return ajustarAncho(_TipoTransporte, 250);
            }
            set
            {
        ...
        public static AdcDoc row2AdcDoc(DataRow r)
        {
            // asigna a un objeto AdcDoc los datos del dataRow indicado
            AdcDoc oAdcDoc = new AdcDoc();
            //
        ...
        private static void AdcDoc2Row(AdcDoc oAdcDoc, DataRow r)
        {
        ...
        private static void nuevoAdcDoc(DataTable dt, AdcDoc oAdcDoc)
        {
            // Crear un nuevo AdcDoc
            DataRow dr = dt.NewRow();
            AdcDoc oA = row2AdcDoc(dr);
            //
        ...
        //
        public static AdcDoc Buscar(string sWhere)
        {
        ...
        public string Crear()
        {
			...
			string codProveedor = "";
            if (Doc_TipoDoc == "FAP") { codProveedor = Doc_codper; } else { codProveedor = ""; };
            ...
            if (!NumeracDoc.FijarNroDeDocumento("ADCDOC", autonumeracion, codProveedor, id, cadenaConexion)) return "ERROR - NUMERO DUPLICADO";
            ...
            if (Doc_TipoDoc == "FAP")
			{
                CadenaSelect = "SELECT * FROM AdcDoc WHERE doc_sucursal = '" + Doc_sucursal + "' and opc_documento = '" + Opc_documento + "' and Doc_codper = '" + Doc_codper + "' and idclavedoc = " + IdClaveDoc.ToString();
            }
			else
			{
