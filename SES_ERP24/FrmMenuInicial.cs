...
namespace SES_ERP24
{
    public partial class FrmMenuInicial : Form
    {
        ...
        private bool _filtroAplicado = false;
        ...
        public FrmMenuInicial()
        {
            ...
            // ============================================================
            // CARGAR LICENCIA DESDE LA BASE DE DATOS
            // ============================================================
            CargarLicenciaDesdeBD();

            // Aplicar filtro ANTES de cargar la información visual
            AplicarFiltroPermisosAutomatico();

            cargarInformacionVisual();

            this.Load += FrmMenuInicial_LoadPermisos;
            this.Shown += FrmMenuInicial_Shown;
        }

        private void CargarLicenciaDesdeBD()
        {
            try
            {
                string ruc = DattCom.datosEmpresa.Emp_RUC;
                if (string.IsNullOrEmpty(ruc))
                {
                    System.Diagnostics.Debug.WriteLine("❌ RUC vacío, no se puede cargar licencia");
                    return;
                }
            ...
            ORDER BY Id DESC";
                ...
                using (var conn = new System.Data.SqlClient.SqlConnection(DattCom.datosEmpresa.strConxSyscod))
                {
                    conn.Open();
                    using (var cmd = new System.Data.SqlClient.SqlCommand(sql, conn))
                    {
                        ...
                        using (var reader = cmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                DattCom.datosEmpresa.TipoLicencia = Convert.ToInt32(reader["TipoLicencia"]);
                                DattCom.datosEmpresa.MaxUsuarios = Convert.ToInt32(reader["MaxUsuarios"]);
                                DattCom.datosEmpresa.FechaExpiracion = Convert.ToDateTime(reader["FechaExpiracion"]);
        ...
        private void FrmMenuInicial_Shown(object sender, EventArgs e)
        {
            AplicarFiltroPermisosAutomatico();
        ...
        #region Métodos Automáticos de Permisos
        private HashSet<string> ObtenerPermisosUsuario()
        {
            System.Diagnostics.Debug.WriteLine("=== ObtenerPermisosUsuario() ===");

            if (_permisosCache != null)
            {
                System.Diagnostics.Debug.WriteLine($"✅ Permisos en caché: {_permisosCache.Count} módulos");
            ...
            if (!string.IsNullOrEmpty(DattCom.datosEmpresa.ModulosActivos) && DattCom.datosEmpresa.TipoLicencia > 0)
            {
                // ============================================================
                // TODOS LOS TIPOS DE LICENCIA (INCLUYENDO MONO) RESPETAN ModulosActivos
                // ============================================================
                _permisosCache = ObtenerPermisosDesdeLicencia();
                System.Diagnostics.Debug.WriteLine($"✅ {_permisosCache.Count} módulos activos según licencia");
                return _permisosCache;
            }

            // FALLBACK - Usar sys_Accesos (comportamiento original)
            System.Diagnostics.Debug.WriteLine("⚠️ FALLBACK: Usando sys_Accesos");
            try
            {
            ...
            System.Diagnostics.Debug.WriteLine($"✅ FALLBACK: {_permisosCache.Count} módulos desde sys_Accesos");
            return _permisosCache;
        }

        private HashSet<string> ObtenerPermisosDesdeLicencia()
        {
            System.Diagnostics.Debug.WriteLine("=== ObtenerPermisosDesdeLicencia() ===");

            var permisos = new HashSet<string>();
            string modulosActivos = DattCom.datosEmpresa.ModulosActivos;

            if (string.IsNullOrEmpty(modulosActivos) || modulosActivos.Length < 35)
            {
                System.Diagnostics.Debug.WriteLine($"❌ ModulosActivos inválido: '{modulosActivos}'");
                return permisos;
            }

            System.Diagnostics.Debug.WriteLine($"📌 ModulosActivos: {modulosActivos}");

            var mapaClaves = ObtenerMapaClaves();
            System.Diagnostics.Debug.WriteLine($"📌 Mapa de claves: {mapaClaves.Count} entradas");

            // ============================================================
            // LOG: Mostrar qué posiciones están activas
            // ============================================================
            System.Diagnostics.Debug.WriteLine("📋 POSICIONES ACTIVAS:");
            for (int i = 0; i < modulosActivos.Length && i < 35; i++)
            {
                if (modulosActivos[i] == '1')
                {
                    System.Diagnostics.Debug.WriteLine($"  Posición {i}: ACTIVA");
                }
            }

            foreach (var item in mapaClaves)
            {
                string clave = item.Key;
                int posicion = item.Value;

                if (posicion >= 0 && posicion < modulosActivos.Length && modulosActivos[posicion] == '1')
                {
                    permisos.Add(clave);
                    System.Diagnostics.Debug.WriteLine($"  ✅ Activo: {clave} (posición {posicion})");
                }
            }

            System.Diagnostics.Debug.WriteLine($"✅ Total módulos activos: {permisos.Count}");
            return permisos;
        }

        private HashSet<string> ObtenerTodasLasClavesMenu()
        {
            System.Diagnostics.Debug.WriteLine("=== ObtenerTodasLasClavesMenu() ===");
            ...
            {
                string strConx = DattCom.datosEmpresa.strConIniSis;
                string ssql = "SELECT Clave FROM MenuSES WHERE Clave IS NOT NULL AND Clave != ''";

                DataTable dt = DattCom.SqlDatos.leerTabla(ssql, strConx);

                if (dt != null)
                {
                    foreach (DataRow row in dt.Rows)
                    {
                        string clave = row["Clave"].ToString().Trim();
            ...
            {
                System.Diagnostics.Debug.WriteLine($"❌ Error obteniendo claves de MenuSES: {ex.Message}");
            }

            System.Diagnostics.Debug.WriteLine($"✅ Total claves MenuSES: {claves.Count}");
            return claves;
        }
        ...
        private void AplicarFiltroPermisosAutomatico()
        {
            ...
            System.Diagnostics.Debug.WriteLine("╔════════════════════════════════════════════════════════════╗");
            System.Diagnostics.Debug.WriteLine("║        APLICAR FILTRO DE PERMISOS                        ║");
            System.Diagnostics.Debug.WriteLine("╚════════════════════════════════════════════════════════════╝");

            bool tieneLicencia = !string.IsNullOrEmpty(DattCom.datosEmpresa.ModulosActivos) &&
                                 DattCom.datosEmpresa.TipoLicencia > 0 &&
                                 DattCom.datosEmpresa.FechaExpiracion >= DateTime.Now;

            System.Diagnostics.Debug.WriteLine($"📌 TipoLicencia: {DattCom.datosEmpresa.TipoLicencia}");
            System.Diagnostics.Debug.WriteLine($"📌 ModulosActivos: {DattCom.datosEmpresa.ModulosActivos}");
            System.Diagnostics.Debug.WriteLine($"📌 FechaExpiracion: {DattCom.datosEmpresa.FechaExpiracion}");
            System.Diagnostics.Debug.WriteLine($"📌 tieneLicencia: {tieneLicencia}");

            if (!tieneLicencia)
            {
                System.Diagnostics.Debug.WriteLine("⚠️ NO HAY LICENCIA ACTIVA");
                HashSet<string> permisosFallback = ObtenerPermisosDesdeSysAccesos();
                if (permisosFallback.Count > 0)
                {
                    System.Diagnostics.Debug.WriteLine($"✅ Usando fallback: {permisosFallback.Count} módulos");
                    AplicarFiltroPorPermisos(permisosFallback);
                    return;
                }

                MessageBox.Show("No se encontró una licencia válida para el sistema.\n\n" +
                              "Contacte a su proveedor para activar el sistema.",
            ...
            System.Diagnostics.Debug.WriteLine($"📌 Permisos obtenidos: {permisos?.Count ?? 0}");

            if (permisos == null || permisos.Count == 0)
            {
                System.Diagnostics.Debug.WriteLine("⚠️ No se obtuvieron permisos");
                permisos = ObtenerPermisosDesdeSysAccesos();

                if (permisos.Count == 0)
                {
                    MessageBox.Show("La licencia no tiene módulos activos.\n\n" +
                                  "Contacte a su proveedor para activar los módulos.",
                                  "Licencia sin módulos",
                                  MessageBoxButtons.OK,
                                  MessageBoxIcon.Warning);
                    Application.Exit();
                    return;
            ...
            System.Diagnostics.Debug.WriteLine("");
            System.Diagnostics.Debug.WriteLine("📋 PERMISOS DEL USUARIO:");
            foreach (var p in permisos.OrderBy(x => x))
            {
                System.Diagnostics.Debug.WriteLine($"  ✅ {p}");
            }
            System.Diagnostics.Debug.WriteLine($"Total: {permisos.Count} módulos");
            System.Diagnostics.Debug.WriteLine("");

            AplicarFiltroPorPermisos(permisos);
        }

        private HashSet<string> ObtenerPermisosDesdeSysAccesos()
        {
            System.Diagnostics.Debug.WriteLine("=== ObtenerPermisosDesdeSysAccesos() ===");
            ...
            {
                System.Diagnostics.Debug.WriteLine($"❌ Error obteniendo permisos de sys_Accesos: {ex.Message}");
            }

            System.Diagnostics.Debug.WriteLine($"✅ sys_Accesos: {permisos.Count} módulos");
            return permisos;
        }

        private void AplicarFiltroPorPermisos(HashSet<string> permisos)
        {
            System.Diagnostics.Debug.WriteLine("=== AplicarFiltroPorPermisos() ===");

            if (permisos == null || permisos.Count == 0)
            {
                System.Diagnostics.Debug.WriteLine("⚠️ No hay permisos, ocultando todo");
            ...
            foreach (ToolStripMenuItem menuPrincipal in menugeneral.Items)
            {
                ...
                foreach (ToolStripItem subItem in menuPrincipal.DropDownItems)
                {
                    if (subItem is ToolStripMenuItem subMenu)
                    {
                        ...
                        bool tieneTag = !string.IsNullOrEmpty(tag);
                        bool permitido = tieneTag && permisos.Contains(tag);

                        System.Diagnostics.Debug.WriteLine($"  📌 Submenú: '{texto}', Tag: '{tag ?? "SIN TAG"}'");

                        if (tieneTag)
                        {
                            System.Diagnostics.Debug.WriteLine($"     📌 ¿Está en permisos? {(permitido ? "✅ SÍ" : "❌ NO")}");
                        }
                        else
                        {
                            System.Diagnostics.Debug.WriteLine($"     ⚠️ SIN TAG - Se oculta");
                        }

                        subMenu.Visible = permitido;
                        if (permitido)
                        {
                ...
                if (tienePermisoEnEsteMenu)
                {
                    ...
                    System.Diagnostics.Debug.WriteLine($"  📁 Menú principal VISIBLE: {menuPrincipal.Text}");
                }
                else
                {
                    System.Diagnostics.Debug.WriteLine($"  📁 Menú principal OCULTO: {menuPrincipal.Text}");
                }
            }

            System.Diagnostics.Debug.WriteLine("=== FIN DE APLICAR FILTRO ===");
            System.Diagnostics.Debug.WriteLine("");
