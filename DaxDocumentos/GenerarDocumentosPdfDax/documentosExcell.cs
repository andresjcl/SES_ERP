                ...
                foreach (DataRow row in datosForma.Rows)
                    {
                        ...
                        { 
                            try { version = row["version"].ToString(); }catch{ }
                            leerDefinicionFormato.LeerLinea(row["l1"].ToString(), ref campForma,0, version );
...
//                    codBarras.registraBarras(Aimprimir, FORMATO, Ancho, alto, Currenty, Posicion, Largoimprimible, ref docmtoPdf);
            ...
            private string DatoGeneral(string dato)
            {
                ...
                if (datNum < 9821 && datNum > 9800 && ImpText != null) return ImpText[datNum - 9801];
                return "";
            }
