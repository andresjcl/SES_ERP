    ...
    static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (parm.IndexOf("^") >= 0 || parm.IndexOf("|") >= 0)
            {
                ...
                {
                    adcDocumentos.FormFactProveedor prog = new adcDocumentos.FormFactProveedor("FAP", "FAP", false);
                    prog.ShowDialog();
            ...
            else
            {
                ...
                SysEmpDatt.IngresoApp.iniciar(parm);
                Syschk ingreso = new Syschk(" EMISION DE FACTURAS MASIVAS ");
                ingreso.ShowDialog();
                if (datosEmpresa.usr == "")
                {
                    return;
                }
                //                adcDocumentos.FormDevolucion   prog = new adcDocumentos.FormDevolucion ("DEVNCC","NCC",false,false);
                adcDocumentos.FormFacMasivva prog = new adcDocumentos.FormFacMasivva();
                prog.ShowDialog();
                prog.Dispose();
            }
